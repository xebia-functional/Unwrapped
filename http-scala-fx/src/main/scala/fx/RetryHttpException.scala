...
opaque type RetryPolicyHttpException[A] = RetryPolicyHttpException.UnhandledRetryPolicyException |
  RetryPolicyHttpException.RetriesExhaustedException[A]

object RetryPolicyHttpException:
  private[fx] final case class UnhandledRetryPolicyException(message: String, cause: Throwable | Null)
      extends RuntimeException(message, cause)
  private[fx] final case class RetriesExhaustedException[A](r: HttpResponse[A])(using Show[HttpResponse[A]])
