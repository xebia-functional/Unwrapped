...
//  println(new ExampleClass {}.continuations(1, 2))
//  println(new ExampleClass2Impl("AAA").continuations(1, 2))

abstract class ExampleClass:
  import ExampleClass.p
  import ExampleClass.method1
  ...
  def continuations(x: Int, y: Int)(using s: Suspend): Int =
    ...
    val r =
      summon[Suspend].suspendContinuation[Int](
        _.resume(Right(x + q + ExampleClass.p + method1(y) + method2(y) + qq)))
    r + y + ExampleClass.p + q + method1(x) + method2(y) + qq
