...
import continuations.ContinuationsPhase.JumpToLabelNumberKey
import continuations.ContinuationsPhase.JumpToLabelNumber
import continuations.ContinuationsPhase.JumpLabelsKey
import continuations.ContinuationsPhase.JumpLabels
  ...
  private def transformContinuationWithSuspend(tree: tpd.ValOrDefDef)(using Context): tpd.Tree =
    ...
    def transformSuspensionsSuspendingStateMachine(
      ...
      if suspensionPoints.isEmpty then tree
        ...
        val transformedTree =
          transformSuspensionsSuspendingStateMachineAux(tree, suspensionInReturnedValue)
      ...
      treeMap = {
        case t @ tpd.DefDef(_, _, _, _) if t.symbol.showFullName == tree.symbol.showFullName =>
          val defWithUpdatedRhs = cpy.DefDef(t)(rhs = tpd.Block(
            reservedVariables ++ List(frameClass.buildInitializer(t)),
            ...
            thickets
              .map { thicket =>
                ...
                body.foreachSubTree(t =>
                  t.putAttachment(JumpToLabelNumberKey, JumpToLabelNumber(currentLabel + 1)))
                tpd.CaseDef(
...
//     transformedMethod
//       .termParamss
//       .map(_.filterNot(_.symbol.info.hasClassSymbol(requiredClass(continuationFullName))))
...
//     transformedMethodParamsWithoutCompletion.flatten.collect { case vd: tpd.ValDef => vd }
...
//             transformedMethodParamsWithoutCompletion.flatten.exists(
