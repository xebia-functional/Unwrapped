...
sealed trait Suspend:
  ...
  extension (s: Suspend)
    def suspendContinuation[A](f: Continuation[A] => Unit): A =
      throw CompilerRewriteUnsuccessfulException

object Suspend:
  case object CompilerRewriteUnsuccessfulException
      extends Exception(
        """|
           |The body of this method should have been generated by the continuations-plugin.
           |Please open an issue so we can fix this:
           |https://github.com/47deg/TBD/issues/new?assignees=&labels=bug&template=compilerrewriteunsuccessfulexception-bug-report.md&title=CompilerRewriteUnsuccessfulException+Bug
           |""".stripMargin)
