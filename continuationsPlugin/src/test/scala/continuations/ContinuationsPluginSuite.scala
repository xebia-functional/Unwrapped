    ...
    case given Context =>
      val source =
        """|
           ...
           |
           |def foo()(using Suspend): Int =
           |  summon[Suspend].shift[Int] { continuation => continuation.resume(Left(new Exception("error"))) }
           |""".stripMargin
    ...
    implicit givenContext =>
      ...
      val source =
        """|
           ...
           |  summon[Suspend].shift[Int] { continuation => continuation.resume(Right(x + 1)) }
           |""".stripMargin
    ...
    case given Context =>
      val source =
        """|
           ...
           |def program: Int = {
           |  def foo(x: Int)(using Suspend): Int = {
           |    summon[Suspend].shift[Int] { continuation => continuation.resume(Right(x)) }
           |    10
           |  }
           |  foo(11)
           |}
    ...
    case given Context =>
      val source =
        """|
           ...
           |  def foo()(using Suspend): Int = {
           |    summon[Suspend].shift[Int] { continuation => continuation.resume(Right(1)) }
           |    summon[Suspend].shift[Boolean] { continuation => continuation.resume(Right(false)) }
           |    summon[Suspend].shift[String] { continuation => continuation.resume(Right("Hello")) }
           |    10
           |  }
           |
           |foo()
    ...
    case given Context =>
      ...
      val sourceSuspend =
        """|
           ...
           |    summon[Suspend].shift[A] { continuation => continuation.resume(Right(x)) }
           |  foo(1,2)("A")
           |}
           |""".stripMargin
    ...
    case given Context =>
      val source =
        """|
           ...
           |
           |def foo(x: Int)(using Suspend): Int = {
           |  val y = summon[Suspend].shift[Int] { continuation => continuation.resume(Right(1)) }
           |  x + y
           |}
           |""".stripMargin
    ...
    case given Context =>
      val source =
        """|
           ...
           |  summon[Suspend].shift[Unit] { _.resume(Right { println(qq) }) }
           |  10
           |}
           |""".stripMargin
    ...
    case given Context =>
      val source =
        """|
           ...
           |  val xx = 111
           |  println(xx)
           |  summon[Suspend].shift[Int] { _.resume(Right( 10 )) }
           |  xx
           |}
           |""".stripMargin
    ...
    case given Context =>
      val source =
        """|
           ...
           |    val pp = 11
           |    val xx = s.shift[Int] { _.resume(Right { qq - 1 }) }
           |    val ww = 13
           |    val rr = "AAA"
           |    val yy = s.shift[String] { _.resume(Right { rr }) }
           |    val tt = 100
           |    val zz = s.shift[Int] { _.resume(Right { ww - 1 }) }
           |    println(xx)
    ...
    case given Context =>
      val source =
        """|
           ...
           |    val ww = 13
           |    val rr = "AAA"
           |    s.shift[String] { _.resume(Right { rr }) }
           |    val tt = 100
           |    val zz = s.shift[Int] { _.resume(Right { ww - 1 }) }
           |    println(xx)
           |    xx + qq + zz + pp + tt
    ...
    case given Context =>
      val source =
        """|
           ...
           |    println("Hello")
           |    val y = s.shift[Int] { _.resume(Right { println("World"); 1 }) }
           |    val z = 1
    ...
    case given Context =>
      val source =
        """|
           ...
           |    val x = s.shift[Int] { _.resume(Right { 1 }) }
           |    s.shift[Int] { _.resume(Right { x + 1 }) }
           |}
           |""".stripMargin
    ...
    case given Context =>
      val source =
        """|
           ...
           |    println("Hello")
           |    val z = 100
           |    val x = s.shift[Int] { _.resume(Right { 1 + z}) }
           |    val j = 9
           |    val w = s.shift[Int] { _.resume(Right { x + 1 + q}) }
           |    println("World")
           |    s.shift[Int] { _.resume(Right { x + w + 1 + j}) }
           |    10
           |}
           |""".stripMargin
