    ...
    case given Context =>
      val source =
        """
          ...
          |      val z = shift[A] { _.resume(a) }
          |      shift[A] { _.resume({ println("World"); z }) }
          |  }
          |
          |  fooTest(Foo(1), 1)
          |}
    ...
    case given Context =>
      val source =
        """|
           ...
           |def fooTest(x: Int, y: Int)(using s: Suspend): Int = {
           |    val w = 1
           |    val z = shift[Int] { _.resume( { x + y + w }) }
           |    shift[Int] { _.resume( { z + 1 }) }
           |}
           |""".stripMargin
    ...
    case given Context =>
      val source =
        """|
           ...
           |    val a = 1
           |    val b = 1
           |    val z = shift[Int] { _.resume(x + y + a) }
           |    val c = a + 1
           |    shift[Int] { _.resume(z + b + c) }
           |}
           |""".stripMargin
    ...
    case given Context =>
      val source =
        """|
           ...
           |    val c = a + b
           |    val d = c + 1
           |    val w = shift[Int] { _.resume(z + c + d) }
           |    println(w)
           |}
           |""".stripMargin
