...
class ContextFunctionsSnapshotSuite extends SnapshotSuite, CompilerFixtures:

  def snapshotValue(tree: Tree)(using Context): String =
    s"""${compileSourceIdentifier.replaceAllIn(tree.toString, "")}
       |
       |${compileSourceIdentifier.replaceAllIn(tree.show, "")}""".stripMargin
