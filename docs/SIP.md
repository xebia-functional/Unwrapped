...
suspend def getCountryCodeDirect(futurePerson: Future[Person])(using Control[NotFound.type | None.type]): String =
...
```scala
suspend def getCountryDirect2(futurePerson: Future[Person])(using Control[NotFound.type | None.type]): String =
...
More complex abstractions can be derived from continuations.
One of them is monad bind. Here monad bind has the shape `F[A] => A`. Once we have a function like `bind`, we can extract `A`
...
Now that we have the implementations for `bind` we can express `countryCode` in a direct, non-blocking style.
...
`Control` provides `shift` the operation that allows interruption analogous to the imperative `throw`:
...
We can `try`/`catch` over them because they are suspension points, and they guarantee control flow will return to the caller either
through a result or an exception in the case of `shift`, yet the work performed may go async, get scheduled, or sleep, all in a non-blocking way.
...
We have taken a couple from the original [coroutines proposal](https://github.com/Kotlin/KEEP/blob/master/proposals/coroutines.md#use-cases) and adapted them to Scala to show how we may use continuations for more than just effects or async operations.
...
suspend def sleep(time: Long): Unit = continuation { c =>
...
```scala
val fibonacci: MutableLazyList[Int] = lazyList {
